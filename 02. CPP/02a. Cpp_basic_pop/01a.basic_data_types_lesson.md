...
- cout: xuất data đã được định dạng.
- cin: nhập data đã được định dạng.
...
cout << "variable a = " << a; // In giá trị của a ra màn hình.
cout << "\nChange a to: ";
cin >> a;        // lấy giá trị ngươi dùng nhập để ghi vào a.
...
- Một số lưu ý về cách sử dụng hàm cout và cin:
    - \n: là ký tự xuống dòng.
    - &a: nghĩa là "địa chỉ của a". Khi sử dụng hàm cin, ta không truyền tên biến vào mà truyền địa chỉ của biến vào.
...
int main(){
    ...
    cout << "c = " << c;    // cũng là một biểu thức.
    return 0;
}
```
Output:
